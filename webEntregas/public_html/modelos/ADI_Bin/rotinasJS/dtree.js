...
// ----------------------------------------------------------------------------
	...
	this.remove = function( id )
	{
		...
		for( i = 0; i < this.arrNodes.length; i++ )
		{
			if( i == id )
				continue;

			tmpArr[ tmpArr.length ] = this.arrNodes[ i ];
		}

		this.arrNodes = tmpArr;
	}
...
// ----------------------------------------------------------------------------
	...
	this.addNode = function(pNode)
	{
		for (var n=0; n<this.arrNodes.length; n++)
		{
			if (this.arrNodes[n].pid == pNode)
			{
				...
				this.strOutput += '<span id="s' + this.instanceName + n + '" class="' + ((this.clickSelect) ? ((n==this.getSelected() ? 'nodeSel' : 'node')) : 'node') + '">';
	...
	{
		sn = this.getCookie('cs' + this.instanceName);
		if( sn )
			return sn;
